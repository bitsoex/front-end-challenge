...
class GraphicMarket extends React.Component {
    ...
    chartClose () {
        ...
        for (let i = 0; i < this.state.data.length-1; i++) {
            ...
            lines.push( <Line points={[ i * pasoX + pasoX/2 , y0, (i+1)*pasoX + pasoX/2, y1]} fill={colorBook} stroke={colorBook} key={i} /> );
        }
        return lines;
    }
    ...
    render () {
        ...
        if (this.state.data.length > 0) {
            ...
            return (
                <div style={{ fontWeight: 'bold' , borderBottom: '1px solid ' + this.props.theme.gray , height: this.props.activeGraphic?'9em':'2em'}} >
                    <div style={{ backgroundColor: this.props.activeGraphic?this.props.theme.activeMarket:this.props.theme.bodyMarkets , 
                        ...
                        <div style={{ display: 'inline-block', position: 'absolute', right: '0px', color: colorBook }}>
                            <NumberFormat value={this.state.data[ this.state.data.length - 1].value} 
                                displayType={'text'} thousandSeparator={true} prefix={'$'} decimalScale={2} suffix={' ' + currency} />
                        </div>
